...
def get_args():
     ...
     parser.add_argument('--exp_label', default='mpc', 
                         help='name of method')
     ...
     parser.add_argument('--num_eval_envs', type=int, default=300, help='number of environments for evaluation during training')
     ...
     parser.add_argument('--vae_storage_max_num_steps', type=int, default=100,
                         help='max number of env steps for vae storage')
     parser.add_argument('--vae_storage_max_buffer_size', type=int, default=100000,
                         help='how many trajectories (!) to keep in vae buffer')
     parser.add_argument('--vae_batch_size', type=int, default=25,
                         help='how many trajectories to use for VAE update')
     parser.add_argument('--vae_reward_reconstruction_loss_coeff', type=float, default=1.0, help='weight for reward loss')
     parser.add_argument('--vae_state_reconstruction_loss_coeff', type=float, default=1.0, help='weight for state loss')
     parser.add_argument('--vae_kl_loss_coeff', type=float, default=0.01, help='weight for the KL term')
     ...
     parser.add_argument('--encoder_layers_before_rnn', nargs='+', type=int, default=[])
     parser.add_argument('--encoder_rnn_hidden_dim', type=int, default=64, 
                         help='dimensionality of RNN hidden state')
     parser.add_argument('--encoder_layers_after_rnn', nargs='+', type=int, default=[])
     parser.add_argument('--encoder_rnn_cell_type', type=str, default='vanilla', 
                         help='choose: vanilla, gru')
     parser.add_argument('--encoder_activation_function', type=str, default='tanh',
                         help='choose: tanh, relu, leaky-relu')
     parser.add_argument('--encoder_initialization_method', type=str, default='normc',
                         help='choose: orthogonal, normc')
     # embed_dim: shared across encoder and decoders
     ...
     parser.add_argument('--state_decoder_layers', nargs='+', type=int, default=[32])
     parser.add_argument('--state_pred_type', type=str, default='deterministic', 
                         help='choose: deterministic, gaussian, bernoulli')
     # reward decoder
     ...
     parser.add_argument('--reward_decoder_layers', nargs='+', type=int, default=[32])
     parser.add_argument('--reward_pred_type', type=str, default='bernoulli',
                         help='choose: '
                              'bernoulli (predict p(r=1|s))'
                              'categorical (predict p(r=1|s) but use softmax instead of sigmoid)'
                              'deterministic (treat as regression problem)')
     ...
     parser.add_argument('--disable_kl_term', type=boolean_argument, default=False,
                         help='dont use the KL regularizing loss term')
     ...
     parser.add_argument('--policy_gradient_through_encoder', type=boolean_argument, default=False,
                         help='to allow policy gradient through encoder')
     parser.add_argument('--policy_gradient_encoder_coeff', type=float, default=1.0,
                         help='weight for the policy gradient through encoder, relative to the VAE loss')
     parser.add_argument('--policy_mlp_use_observations', type=boolean_argument, default=True,
                   help='whether policy MLP takes current observations as input, default True for backward compatibility')
