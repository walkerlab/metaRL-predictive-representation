...
class TBLogger:
    def __init__(self, args, exp_label):
        self.output_name = exp_label + '_' + str(args.seed) + '_' + datetime.datetime.now().strftime('_%d:%m_%H:%M:%S') + '_' + str(args.run_id)
        try:
        ...
        with open(os.path.join(self.full_output_folder, 'config.json'), 'w') as f:
            ...
            for k, v in vars(args).items():
                if k != 'device':
                    if isinstance(v, gym.spaces.discrete.Discrete):
                        config[k] = [v]
                    elif isinstance(v, (np.int_, np.int64)):
                        config[k] = int(v)
                    else:
                        config[k] = v
                #config = {k: v for (k, v) in  if k != 'device'}
            # except:
            #     config = args
